        ...
        window.articles.forEach(async article => {
            ...
            try {
              ...
              const analyticsResponse = await fetch(ANALYTICS_ENDPOINT, {
                ...
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${ANON_KEY}`,  // Authenticate with anon key
                  'x-client-info': 'xreplygpt-extension/6.2'  // Identify the client
                },
                body: JSON.stringify({
                  user: userHandle,
                  to_user: username,
              ...
              if (analyticsResponse.ok) {
                console.log('✅ Analytics recorded in Supabase cloud (nvcsefnmibvlaxrejrvv)');
              } else {
                const errorText = await analyticsResponse.text();
                console.warn(`⚠️ Analytics failed (${analyticsResponse.status}):`, errorText.substring(0, 200));
              }
            } catch (analyticsError) {
              // Non-critical - don't break reply generation if analytics fails
              console.log('ℹ️ Analytics unavailable (continuing without):', analyticsError.message);
            }

            let buttonReply = document.createElement("button");
            buttonReply.id = "generated-reply";
            buttonReply.setAttribute("data-link", "https://twitter.com/intent/tweet?text=" + encodeURIComponent(generatedText) + "&in_reply_to=" + tweetId)
